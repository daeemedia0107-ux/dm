...
function Founder() {
  const content = window.CMS_CONTENT?.founder || {};
  const eyebrow = content.eyebrow || "The Founder";
  const name = content.name || "Saad";
  const bio = content.bio || [
    "I started Daee because most performance marketing agencies run ads without understanding the business underneath them.",
    ...
    <Section id="about" pad={160} bg="var(--ink-900)" style={{ color: 'var(--paper-50)', position: 'relative', overflow: 'hidden' }}>
      ...
      <Container>
        <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 80, alignItems: 'start' }}>
          <Reveal>
            <FounderPortrait image={content.image} name={name} />
          </Reveal>
            ...
            <div>
              ...
              <div style={{ fontSize: 18, lineHeight: 1.7, color: 'var(--paper-200)', marginTop: 44, maxWidth: 640 }}>
                {bio.map((paragraph, index) => (
                  <p key={index} style={{ margin: index === bio.length - 1 ? 0 : '0 0 22px' }}>
                    {paragraph}
                  </p>
                ))}
              </div>
...
function FounderPortrait({ image, name }) {
  return (
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            color: 'var(--paper-300)', fontWeight: 500, mixBlendMode: image ? 'difference' : 'normal',
          }}>Founder portrait</div>
          <div>
            <div style={{
              fontFamily: 'var(--font-display)', fontSize: 56, lineHeight: 1,
              color: image ? 'var(--paper-50)' : 'var(--olive-300)', fontStyle: 'italic', letterSpacing: '-0.02em',
              mixBlendMode: image ? 'difference' : 'normal',
            }}>{name.split(' ')[0]}</div>
          </div>
        </div>
      </div>
